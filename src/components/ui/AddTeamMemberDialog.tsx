      ...
      <div className="absolute inset-0 sm:inset-auto sm:top-[10%] sm:left-1/2 sm:-translate-x-1/2 sm:max-w-lg sm:w-full bg-white sm:rounded-2xl shadow-2xl overflow-hidden animate-fadeIn flex flex-col max-h-screen sm:max-h-[80vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-[#082545]" style={{ fontFamily: 'var(--font-heading)' }}>Ajouter un membre</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.prenom} onChange={e => set('prenom', e.target.value)} className={`w-full px-3 py-2.5 rounded-lg border text-sm min-h-[44px] outline-none ${errors.prenom ? 'border-red-300' : 'border-gray-200 focus:border-[#2EC6F3]'}`} />
              {errors.prenom && <p className="text-red-500 text-xs mt-1">{errors.prenom}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nom *</label>
              <input value={form.nom} onChange={e => set('nom', e.target.value)} className={`w-full px-3 py-2.5 rounded-lg border text-sm min-h-[44px] outline-none ${errors.nom ? 'border-red-300' : 'border-gray-200 focus:border-[#2EC6F3]'}`} />
          ...
          <div>
            ...
            <input type="email" value={form.email} onChange={e => set('email', e.target.value)} className={`w-full px-3 py-2.5 rounded-lg border text-sm min-h-[44px] outline-none ${errors.email ? 'border-red-300' : 'border-gray-200 focus:border-[#2EC6F3]'}`} />
            {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Téléphone</label>
            <input value={form.telephone} onChange={e => set('telephone', e.target.value)} placeholder="06 12 34 56 78" className={`w-full px-3 py-2.5 rounded-lg border text-sm min-h-[44px] outline-none ${errors.telephone ? 'border-red-300' : 'border-gray-200 focus:border-[#2EC6F3]'}`} />
          ...
          {form.role === 'formatrice' && (
            <div>
              ...
              <input type="number" value={form.taux_horaire} onChange={e => set('taux_horaire', parseFloat(e.target.value) || 0)} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm min-h-[44px] outline-none focus:border-[#2EC6F3]" />
            </div>
          )}
          ...
          {form.role !== 'formatrice' && (
            <div>
              ...
              <input type="number" value={form.objectif_mensuel} onChange={e => set('objectif_mensuel', parseInt(e.target.value) || 0)} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm min-h-[44px] outline-none focus:border-[#2EC6F3]" />
            </div>
          )}

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Spécialités (séparées par virgule)</label>
            <input value={form.specialites} onChange={e => set('specialites', e.target.value)} placeholder="microblading, full lips, soins visage" className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm min-h-[44px] outline-none focus:border-[#2EC6F3]" />
          </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-end gap-3 shrink-0">
          ...
          <button onClick={() => createMember.mutate()} disabled={createMember.isPending} className="px-5 py-2.5 bg-[#2EC6F3] hover:bg-[#1BA8D4] text-white rounded-xl text-sm font-medium transition disabled:opacity-50 min-h-[44px] flex items-center gap-2">
