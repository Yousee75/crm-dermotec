...
function CollapsibleBlock({ icon, title, children, defaultOpen = false, badge }: {
  ...
  return (
    <div className="border border-[#F0F0F0] rounded-lg overflow-hidden">
      <button onClick={() => setOpen(!open)} className="flex items-center justify-between w-full px-3 py-2 hover:bg-[#FAFAFA] transition">
      ...
      {open && <div className="px-3 pb-3 pt-1 border-t border-[#FAFAFA]">{children}</div>}
    </div>
  )
}
            ...
            <div className="mt-2 pt-2 border-t border-[#F0F0F0]">
              ...
              <div className="flex flex-wrap gap-1">
                {e.reviews.placeData.subtypes.map((t: string, i: number) => (
                  <span key={i} className="text-[10px] bg-[#F5F5F5] text-[#777777] px-2 py-0.5 rounded-full">{t}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <CollapsibleBlock icon={<MapPin />} title="Environnement Local" badge={`Trafic ${e.quartier.footTrafficScore}/100`}>
          <div className="grid grid-cols-4 gap-2 mb-2">
            {[
              ...
              <div key={i} className="text-center bg-[#FAFAFA] rounded-lg p-2">
