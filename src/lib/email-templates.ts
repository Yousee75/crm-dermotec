    ...
    html: layout([
      ...
      infoBox([
        ...
        iconList([
          ...
          { icon: '⭐', text: 'Ce que disent nos anciennes élèves (4.9/5 sur Google)' },
        ]),
      ].join(''), 'info'),
...
export function emailConfirmationInscription(data: {
  ...
  return {
    ...
    html: layout([
      ...
      data.portail_url ? button('Accéder à mon espace personnel', data.portail_url) : '',
      signature(),
...
export function emailCertificat(data: {
  ...
  return {
    ...
    html: layout([
      ...
      paragraph(`Votre certificat et votre attestation de fin de formation sont disponibles dans votre espace personnel.`),
    ...
    html: layout([
      ...
      data.prochaine_session ? infoBox([
        iconList([
          ...
          { icon: '💰', text: '80% de nos élèves sont financées à 100%' },
          { icon: '⭐', text: '4.9/5 sur Google — +500 professionnelles formées' },
        ]),
      ].join(''), 'info') : '',
      paragraph(`Si le budget vous freinait, sachez qu'on accompagne toutes nos élèves pour monter leur dossier de financement — et c'est gratuit. 80% sont financées à 100%.`),
