  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        // Session pleine
        else if (action === 'session_pleine' && record.metadata?.pourcentage) {
          toast.info('📅 Session presque pleine', {
            description: description.replace('[Agent IA] ', ''),
            duration: 8000,
            action: record.session_id ? {
              label: 'Voir la session',
        ...
        // Prospect sans formation
        else if (action === 'prospect_sans_formation' && record.metadata?.score) {
          toast.warning('🎯 Prospect sans formation', {
            description: description.replace('[Agent IA] ', ''),
            duration: 8000,
            action: record.lead_id ? {
              label: 'Voir le prospect',
        ...
        // Rappel en retard
        else if (action === 'rappel_overdue' && record.metadata?.retard_heures) {
