    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-medium text-[#0EA5E9]">{progress}%</p>
            </div>
          </div>
          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-[#0EA5E9] rounded-full transition-all duration-300" style={{ width: `${progress}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        {data.questions.sort((a, b) => a.ordre - b.ordre).map((q, idx) => (
          <div key={q.id} className="bg-white rounded-xl shadow-sm border p-5">
            <div className="flex items-start gap-3 mb-4">
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-[#0EA5E9]/10 text-[#0EA5E9] text-sm font-bold flex items-center justify-center">
                {idx + 1}
              </span>
              <div>
                ...
                {Array.from({ length: 10 }, (_, i) => i + 1).map(n => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg text-sm font-medium transition-all ${
                      reponses[q.id] === n
                        ? 'bg-[#0EA5E9] text-white shadow-md scale-110'
                ...
                {q.options.map(opt => (
                  <button
                    ...
                    className={`w-full text-start px-4 py-3 rounded-lg text-sm transition-all border ${
                      reponses[q.id] === opt
                        ? 'bg-[#0EA5E9]/5 border-[#0EA5E9] text-[#0EA5E9] font-medium'
            ...
            {q.type === 'texte_libre' && (
              <textarea
                ...
                rows={3}
                className="w-full rounded-lg border border-gray-200 px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#0EA5E9]/20 focus:border-[#0EA5E9]"
                style={{ fontSize: '16px' }}
              />
            )}
          </div>
        ...
        <button
          ...
          disabled={submitting}
          className="w-full bg-[#0EA5E9] hover:bg-[#0284C7] text-white font-medium py-4 rounded-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-base"
