...
export default function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-3">
      {/* ===== HEADER + ACTIONS (toujours visible) ===== */}
      <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4">
        {/* Ligne 1 : Back + Nom + Statut + Score + Actions */}
        <div className="flex items-center gap-2 sm:gap-3">
          <Link href="/leads" className="p-1 hover:bg-gray-100 rounded-lg transition shrink-0"><ArrowLeft className="w-4 h-4 text-gray-400" /></Link>
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-white font-semibold text-xs shrink-0" style={{ backgroundColor: scoreColor }}>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <div className="relative">
                ...
                {showStatutMenu && validTransitions.length > 0 && (
                  ...
                  <div className="absolute left-0 top-full mt-1 w-40 bg-white rounded-lg shadow-xl border z-50 py-1 animate-scaleIn">
                    {validTransitions.map(sk => (
                      <button key={sk} onClick={() => handleChangeStatut(sk)} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-gray-50 text-left">
            ...
            <div className="flex items-center gap-2 text-[11px] text-gray-500 mt-0.5 truncate">
              ...
              {lead.entreprise_nom && <span className="text-gray-300">·</span>}
        ...
        <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-50">
          ...
          <div className="relative">
            <button onClick={() => setShowMoreActions(p => !p)} className="flex items-center justify-center w-9 h-9 bg-gray-50 text-gray-500 rounded-lg hover:bg-gray-100 transition"><MoreHorizontal className="w-4 h-4" /></button>
            {showMoreActions && (
              <><div className="fixed inset-0 z-40" onClick={() => setShowMoreActions(false)} />
              <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-xl shadow-xl border z-50 py-1 animate-scaleIn origin-top-right">
                <button onClick={() => { setShowMoreActions(false); setShowWizard(true) }} className="w-full flex items-center gap-2.5 px-3 py-2 text-xs hover:bg-gray-50 text-left"><GraduationCap className="w-4 h-4 text-violet-500" />Inscrire à une formation</button>
                <button onClick={() => { setShowMoreActions(false); setShowInscrire(true) }} className="w-full flex items-center gap-2.5 px-3 py-2 text-xs hover:bg-gray-50 text-left"><Calendar className="w-4 h-4 text-amber-500" />Inscription rapide</button>
                <button onClick={() => { setShowMoreActions(false); setShowAssign(true) }} className="w-full flex items-center gap-2.5 px-3 py-2 text-xs hover:bg-gray-50 text-left"><Target className="w-4 h-4 text-gray-500" />{lead.commercial_assigne ? `Réassigner` : 'Assigner commercial'}</button>
                <button onClick={() => { setShowMoreActions(false); setIsEditing(true) }} className="w-full flex items-center gap-2.5 px-3 py-2 text-xs hover:bg-gray-50 text-left"><Edit3 className="w-4 h-4 text-gray-500" />Modifier la fiche</button>
              </div></>
            )}
          </div>
        </div>
      ...
      {/* ===== LAYOUT — Desktop: 2 colonnes (contenu gauche, profil droite) | Mobile: 1 colonne ===== */}
      <div className="flex flex-col lg:flex-row-reverse gap-4">

        {/* ── SIDEBAR DROITE (desktop) / EN BAS (mobile) ── */}
        <div className="lg:w-[300px] lg:shrink-0 space-y-3 lg:sticky lg:top-[72px] lg:self-start lg:max-h-[calc(100vh-88px)] lg:overflow-y-auto lg:scrollbar-hide order-2 lg:order-none">
            ...
            <details className="bg-white rounded-xl border border-gray-100 overflow-hidden" open>
              ...
              <div className="px-4 pb-3 space-y-2 border-t border-gray-50 pt-2">
                ...
                {lead.financements?.map((fin: Financement, i: number) => (
                  <div key={i} className="flex justify-between text-[11px] text-amber-700"><span>{fin.organisme}</span><span className="font-medium">{fin.statut.replace(/_/g, ' ')}</span></div>
                ))}
              </div>
            </details>
          )}
        ...
        {/* ── CONTENU PRINCIPAL (gauche desktop, en premier mobile) ── */}
        <div className="flex-1 min-w-0 space-y-3 order-1 lg:order-none">
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div className="px-3 py-2 border-b border-gray-50 bg-gray-50/50">
              <div className="flex items-center gap-1 mb-1.5 overflow-x-auto">
                {CANAUX_MESSAGE.map(c => (
                  <button key={c.id} onClick={() => setMessageCanal(c.id)} className={cn('flex items-center gap-1 px-2 py-1 rounded-md text-[10px] whitespace-nowrap transition', messageCanal === c.id ? 'bg-white shadow-sm font-medium' : 'text-gray-500')}>
                    <c.icon className="w-3 h-3" style={{ color: messageCanal === c.id ? c.color : undefined }} /><span className="hidden sm:inline">{c.label}</span>
                  </button>
                ))}
              </div>
            ...
            <div className="p-3"><ActivityTimeline leadId={lead.id} limit={15} /></div>
          </div>
        </div>
      </div>
